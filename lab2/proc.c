...
exit(void)
{
  ...
  //cprintf("\n this is process %d\n", curproc->pid);
  cprintf("\n pid: %d with a starting time of %d\n", curproc->pid, curproc->T_start);
...
set_prior(int prior_lvl) {
	...
	cprintf("%d is new priority when compared to passed parameter of %d\n", p->prior_val, prior_lvl);
	yield();
	return p->prior_val;
}
